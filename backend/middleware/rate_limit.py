...
def check_rate_limit(session_id: str) -> tuple[bool, int, int]:
    ...
    print(f"[RATE LIMIT] CHECK - Count: {user_data['count']}, Limit: {FREE_TIER_LIMIT}, Allowed: {is_allowed}, Remaining: {remaining}")
...
def increment_usage(session_id: str):
    ...
    if session_id in usage_tracker:
        ...
        print(f"[RATE LIMIT] Incremented count for {session_id} to {usage_tracker[session_id]['count']}")
